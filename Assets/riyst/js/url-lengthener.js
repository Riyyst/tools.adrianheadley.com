...
(function(){
  ...
  // Make URL longer by appending a random query parameter
  const part = Array.from({length:48}, () => Math.random().toString(36).slice(2,3)).join('');
  const param = 'extra=' + part;
  return u + (u.includes('?') ? '&' : '?') + param;
}
